...
unsafe fn public_window_callback_inner(
    ...
    let callback = || match msg {
        ...
        WM_DPICHANGED => {
            ...
            // On Windows 11+ (build >= 22000), the OS-suggested rect from WM_DPICHANGED's lParam
            // is correct on its own; the legacy "conservative_rect" logic below would actively
            // mis-adjust it and trip a feedback loop that bounces the window at monitor DPI
            // boundaries. Cherry-picked from winit PR #4341 (commit 488c036). Only run the
            // legacy adjustment on Windows 10 builds < 22000 where it's still needed.
            if !super::dark_mode::WIN10_BUILD_VERSION.is_some_and(|v| v < 22000) {
